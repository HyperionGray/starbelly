# This module exercises end-to-end subscription behavior against the integration
# test database fixture. Keep it in integration tests because it verifies wiring
# across the websocket protocol, subscription manager, and persistence layer.
...
from starbelly.subscription import JobSyncSubscription
...
async def response_table(db_pool):
    async with db_pool.connection() as conn:
        ...
        await r.table("response").index_create(
            "job_sync", [r.row["job_id"], r.row["sequence"]]
        ).run(conn)
...
@fail_after(3)
async def test_subscribe_to_crawl(
    db_pool, job_table, response_table, response_body_table, nursery
):
    ...
    async with db_pool.connection() as conn:
        await r.table("job").insert(
            {"id": str(job_id), "run_state": RunState.COMPLETED,}
        ).run(conn)
        ...
        await r.table("response").insert(
            {
                ...
                "headers": ["Server", "FakeServer 1.0", "X-Foo", "Bar",],
            }
        ).run(conn)

        await r.table("response_body").insert(
            {"id": b"\x01" * 32, "is_compressed": False, "body": b"File not found",}
        ).run(conn)
        await r.table("response").insert(
            {
...
@fail_after(3)
async def test_subscribe_to_crawl_decompress(
    db_pool, job_table, response_table, response_body_table, nursery
):
...
@fail_after(6)
async def test_subscribe_to_unfinished_crawl(
    db_pool, job_table, response_table, response_body_table, nursery
):
    ...
    async with db_pool.connection() as conn:
        await r.table("job").insert(
            {"id": str(job_id), "run_state": RunState.RUNNING,}
        ).run(conn)
