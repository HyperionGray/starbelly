...
class SubscriptionManager:
    ...
    def subscribe_job_status(self, stats_tracker, min_interval):
        ...
        """
        sub_id = next(self._subscription_id)
        sub = JobStatusSubscription(
            sub_id, self._websocket, stats_tracker, min_interval
        )
        self._subscriptions[sub_id] = sub
    ...
    def subscribe_resource_monitor(self, resource_monitor, history):
        ...
        """
        sub_id = next(self._subscription_id)
        sub = ResourceMonitorSubscription(
            sub_id, self._websocket, resource_monitor, history
        )
        self._subscriptions[sub_id] = sub
...
class JobSyncSubscription:
    ...
    def __init__(
        ...
        if sync_token:
            ...
            logging.debug(
                "%r Setting current sequence to %d", self, self._current_sequence
            )

    def __repr__(self):
        """ For debugging purposes, put the subscription ID and part of the job
        ID in a string. """
        return "<JobSyncSubscription id={} job_id={}>".format(
            self._id, self._job_id[:8]
        )
    ...
    async def _run_sync(self):
        ...
        async for _ in backoff:
            item_count = 0
            async for item in self._db.get_job_sync_items(
                self._job_id, self._current_sequence
            ):
                item_count += 1
    ...
    async def _send_item(self, item_doc):
        ...
        logger.debug(
            "%r Sending item seq=%d url=%s", self, item_doc["sequence"], item_doc["url"]
        )
        message = ServerMessage()
        message.event.subscription_id = self._id
