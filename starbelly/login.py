...
class LoginManager:
    ...
    async def login(self, domain):
        ...
        '''
        login = await self._db.get_login(domain)
        if login is None:
            return
        ...
        request = DownloadRequest(frontier_id=None, job_id=self._job_id, method='GET',
            url=login['login_url'], form_data=None, cost=1.0)
        response = await self._downloader.download(request)
        if not response.is_success:
            logger.error('Login aborted: cannot fetch %s', response.url)
            return
        try:
            action, method, data = await self._get_login_form(response,
