...
### 2. SSL Certificate Verification (Policy-Controlled)

**File:** `starbelly/downloader.py`  
**Line:** 261

**Change:** Added policy-level SSL verification control in downloader:
```python
...
- Added policy-level `verify_ssl` setting (defaults to `False` for compatibility)

**Impact:** Minimal code change; maximum documentation impact.

### 3. Old URL State Serialization Hardening

**File:** `starbelly/job.py`  
**Line:** 328

**Change:** Replaced default pickle serialization with JSON and added safe legacy compatibility:
```python
...
- Paused job state now stores JSON payloads by default
- Legacy pickle payloads are still readable through a restricted unpickler
